...
echo "📦 جاري تحديث نافذة إضافة المنتج لتتطابق 100% مع الصورة ورفع التعديل..."
...
<body class="bg-[#f4f7fb] text-slate-800 min-h-screen flex overflow-x-hidden">
    ...
    <main class="flex-1 p-3 md:p-6 overflow-y-auto custom-scroll">
      ...
      <section id="view-debts" class="page-view hidden space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div class="bg-white p-4 rounded-2xl border shadow-sm">
            <span class="text-xs text-slate-400 font-bold">حالة السداد</span>
      ...
      <section id="view-repairs" class="page-view hidden space-y-4">
        <div class="bg-white p-4 rounded-2xl border shadow-sm flex justify-between items-center">
          <h2 class="text-base font-black text-slate-800">قسم الصيانة</h2>
      ...
      <section id="view-purchases" class="page-view hidden space-y-4">
        ...
        <div class="bg-white rounded-2xl border overflow-hidden shadow-sm">
          <table class="w-full text-right text-xs">
            <thead class="bg-slate-50 text-slate-500 border-b"><tr><th class="p-3">رقم الفاتورة</th><th class="p-3">المورد</th><th class="p-3">الأصناف</th><th class="p-3">الإجمالي</th><th class="p-3">التاريخ</th></tr></thead>
      ...
      <section id="view-customers" class="page-view hidden space-y-4">
        <div class="bg-white p-4 rounded-2xl border shadow-sm"><h2 class="text-base font-black text-slate-800">دليل العملاء</h2></div>
        <div class="bg-white rounded-2xl border overflow-hidden shadow-sm">
          <table class="w-full text-right text-xs">
            <thead class="bg-slate-50 text-slate-500 border-b"><tr><th class="p-3">اسم العميل</th><th class="p-3">الهاتف</th><th class="p-3">إجمالي التعاملات</th><th class="p-3">الرصيد</th></tr></thead>
      ...
      <section id="view-suppliers" class="page-view hidden space-y-4">
        <div class="bg-white p-4 rounded-2xl border shadow-sm"><h2 class="text-base font-black text-slate-800">الموردين والشركات</h2></div>
        <div class="bg-white rounded-2xl border overflow-hidden shadow-sm">
          <table class="w-full text-right text-xs">
            <thead class="bg-slate-50 text-slate-500 border-b"><tr><th class="p-3">اسم المورد</th><th class="p-3">الهاتف</th><th class="p-3">التوريدات</th></tr></thead>
      ...
      <section id="view-treasury" class="page-view hidden space-y-4">
        <div class="bg-white p-4 rounded-2xl border shadow-sm"><h2 class="text-base font-black text-slate-800">الخزينة والمحافظ الإلكترونية</h2></div>
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      ...
      <section id="view-reports" class="page-view hidden space-y-4">
        <div class="bg-white p-4 rounded-2xl border shadow-sm"><h2 class="text-base font-black text-slate-800">التقارير المالية والأرباح</h2></div>
      </section>

      <section id="view-settings" class="page-view hidden space-y-4">
        <div class="bg-[#121c2e] text-slate-200 rounded-3xl border border-slate-800 shadow-2xl p-5 space-y-4">
          ...
          <div class="bg-[#172338] rounded-2xl border border-slate-800 overflow-hidden">
            <table class="w-full text-right text-xs">
              <thead class="bg-[#0c1322] text-slate-400 border-b border-slate-800">
                <tr><th class="p-3">الاسم</th><th class="p-3">اسم المستخدم (Login)</th><th class="p-3">الباسوورد</th><th class="p-3">الصلاحية</th><th class="p-3 text-center">إجراء</th></tr>
              </thead>
  ...
  <div id="modal-add-product" class="fixed inset-0 bg-black/75 backdrop-blur-sm z-55 hidden flex items-center justify-center p-3">
    <div class="bg-white text-slate-800 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden flex flex-col max-h-[94vh] animate-in zoom-in-95">
      ...
      <!-- ترويسة المودال (إضافة منتج مع زر X) -->
      ...
      <!-- الحقول الكاملة المطابقة للصورة بالمللي -->
      ...
      <!-- أزرار الحفظ والإلغاء بالأسفل (مطابقة للصورة) -->
  ...
  <div id="modal-repair-job" class="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 hidden flex items-center justify-center p-3">
    <div class="bg-white rounded-3xl w-full max-w-lg shadow-2xl p-5 space-y-3 text-xs">
      ...
      <div class="grid grid-cols-2 gap-2">
        <input type="text" id="jobDeviceModel" placeholder="الموديل" class="border rounded-xl p-2 font-bold">
        <input type="text" id="jobImei" placeholder="الـ IMEI أو الباسوورد" class="border rounded-xl p-2 font-mono">
      </div>
      <textarea id="jobFault" placeholder="شكوى العميل..." class="w-full border rounded-xl p-2"></textarea>
      <button onclick="alert('تم حفظ كارت الصيانة'); closeModal('modal-repair-job');" class="w-full bg-blue-600 text-white font-black py-2 rounded-xl">حفظ وطباعة</button>
    </div>
  </div>
  ...
  <div id="modal-checkout-pos" class="fixed inset-0 bg-black/75 backdrop-blur-sm z-55 hidden flex items-center justify-center p-3">
    <div class="bg-white rounded-3xl w-full max-w-sm shadow-2xl p-5 space-y-3 text-xs">
      ...
      <div class="flex justify-between font-bold"><span>الإجمالي:</span><span id="checkoutTotalAmount" class="font-black text-emerald-600"></span></div>
      <button onclick="alert('✅ تم البيع بنجاح'); closeModal('modal-checkout-pos'); clearCart();" class="w-full bg-emerald-600 text-white font-black py-2.5 rounded-xl">تأكيد البيع</button>
    </div>
  </div>
  ...
  <script>
    ...
    let App = JSON.parse(localStorage.getItem('EL_RESALA_IMAGE_MODAL_STORE')) || store;

    function saveApp() {
      localStorage.setItem('EL_RESALA_IMAGE_MODAL_STORE', JSON.stringify(App));
    }

    // ================= محرك نافذة إضافة المنتج المطابقة للصورة =================
    let currentUploadedProductImage = '';

    function openAddProductModal() {
      // تفريغ الحقول وتوليد باركود تلقائي مطابق للصورة FG...
    ...
    function generateNewBarcodeCode() {
      const code = 'FG' + Math.floor(1000000 + Math.random() * 9000000);
      document.getElementById('prodBarcodeInput').value = code;
    }
    ...
    function saveProductFromNewModal() {
      ...
      if (!name) {
        return alert('يرجى كتابة اسم المنتج أولاً!');
      }
    ...
    // ================= دوال ريندر المخزون وعرض الأصناف =================
    function renderInventory() {
      ...
      App.products.filter(p => p.name.toLowerCase().includes(q) || (p.barcode && p.barcode.toLowerCase().includes(q))).forEach(p => {
        ...
        tbody.innerHTML += `
          <tr class="hover:bg-slate-50">
            ...
            <td class="p-3 text-center">
              <button onclick="p.stock = parseInt(prompt('تعديل كمية المخزن:', p.stock)); saveApp(); renderInventory();" class="text-blue-600 font-bold hover:underline">تعديل</button>
            </td>
          </tr>
        `;
      });
        ...
        grid.innerHTML += `
          <div onclick="addToCart('${p.id}')" class="bg-slate-50 hover:bg-blue-50/60 border border-slate-200 p-3 rounded-2xl cursor-pointer transition flex flex-col justify-between">
            <div>
              ...
              <p class="text-[10px] text-slate-400 font-mono mt-0.5">${p.barcode || ''}</p>
            </div>
    ...
    function renderCart() {
      ...
      App.cart.forEach((i, idx) => {
        sum += (i.price * i.qty);
        box.innerHTML += `
          <div class="p-2.5 rounded-xl bg-slate-50 border flex justify-between items-center text-xs">
    ...
    function saveUserPasswordChange() {
      ...
      if (!newName || !newLogin || !newPass) return alert('يرجى ملء جميع الحقول!');
      ...
      if (user) {
        ...
        alert(`✅ تم تحديث باسوورد المدير بنجاح إلى: ${newPass}`);
      }
    }
...
git commit -m "feat(inventory): add exact product creation modal matching mockup with image upload, barcode generator and min stock alerts"
git push origin main

echo "=========================================================="
echo "✨ تم تحديث ورفع نافذة إضافة المنتج بنجاح!"
echo "افتح المخزون واضغط (إضافة منتج) لتجد نفس الحقول وخيار توليد الباركود."
echo "=========================================================="
