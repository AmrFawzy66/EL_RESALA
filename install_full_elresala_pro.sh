...
echo "🚀 جاري تثبيت وتحديث نظام EL-RESALA الاحترافي الشامل..."
...
<body class="bg-[#f4f7fb] text-slate-800 min-h-screen flex overflow-x-hidden">
    ...
    <main class="flex-1 p-3 md:p-6 overflow-y-auto custom-scroll">
      ...
      <section id="view-dashboard" class="page-view space-y-5">
        ...
        <div class="bg-white rounded-3xl p-5 border border-slate-200/80 shadow-sm space-y-4">
          <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            <div>
              <h3 class="font-black text-sm text-slate-800 flex items-center gap-2"><i data-lucide="layout-grid" class="w-4 h-4 text-blue-600"></i> الوصول السريع لكافة الأقسام</h3>
              <p class="text-[11px] text-slate-400">انقر على أي قسم لفتحه مباشرة بضغطة زر واحدة</p>
            </div>
          </div>
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 text-center font-bold text-xs">
            ...
            <button onclick="navigateTo('debts')" class="p-3.5 bg-amber-50/70 hover:bg-amber-100 text-amber-700 rounded-2xl flex flex-col items-center gap-2 border border-amber-100 transition shadow-sm hover:scale-105">
            ...
            <button onclick="navigateTo('waste')" class="p-3.5 bg-rose-50/70 hover:bg-rose-100 text-rose-700 rounded-2xl flex flex-col items-center gap-2 border border-rose-100 transition shadow-sm hover:scale-105">
            ...
            <button onclick="navigateTo('reports')" class="p-3.5 bg-indigo-50/70 hover:bg-indigo-100 text-indigo-700 rounded-2xl flex flex-col items-center gap-2 border border-indigo-100 transition shadow-sm hover:scale-105">
            ...
            <button onclick="navigateTo('suppliers')" class="p-3.5 bg-purple-50/70 hover:bg-purple-100 text-purple-700 rounded-2xl flex flex-col items-center gap-2 border border-purple-100 transition shadow-sm hover:scale-105">
              <i data-lucide="building" class="w-6 h-6 text-purple-600"></i><span>الموردين</span>
            </button>
      ...
      <!-- ================= 2. شاشة المبيعات (POS) مع تخصيص السعر والدفع المتعدد ================= -->
      <section id="view-pos" class="page-view hidden space-y-4">
        <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm">
          ...
          <button onclick="exportToExcel(App.products, 'المبيعات')" class="bg-emerald-50 text-emerald-600 border border-emerald-200 text-xs font-bold px-3 py-1.5 rounded-xl flex items-center gap-1">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <div class="lg:col-span-5 bg-white rounded-2xl p-4 border border-slate-200/80 shadow-sm space-y-3">
            ...
            <!-- عناصر السلة مع حقل تعديل السعر المباشر -->
      ...
      <!-- ================= 3. شاشة الديون والآجل (DEDICATED DEBTS PAGE) ================= -->
      <section id="view-debts" class="page-view hidden space-y-4">
        <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm">
          ...
          <button onclick="exportToExcel(App.debts, 'ديون_العملاء')" class="bg-emerald-50 text-emerald-600 border border-emerald-200 text-xs font-bold px-3 py-1.5 rounded-xl flex items-center gap-1.5">
            <i data-lucide="sheet" class="w-3.5 h-3.5"></i> تصدير إكسيل
          </button>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div class="bg-white p-4 rounded-2xl border shadow-sm">
            <span class="text-xs text-slate-400 font-bold">إجمالي المبالغ الآجلة (المتبقية عند العملاء)</span>
            <div class="text-2xl font-black text-rose-500 mt-1" id="totalDebtsAmount">0.00 ج.م</div>
          </div>
          <div class="bg-white p-4 rounded-2xl border shadow-sm">
            <span class="text-xs text-slate-400 font-bold">عدد الفواتير الآجلة</span>
            <div class="text-2xl font-black text-amber-600 mt-1" id="totalDebtsCount">0 فاتورة</div>
          </div>
          <div class="bg-white p-4 rounded-2xl border shadow-sm">
            <span class="text-xs text-slate-400 font-bold">التحصيل الشهري</span>
            <div class="text-2xl font-black text-emerald-600 mt-1">منتظم</div>
          </div>
        </div>
          ...
          <table class="w-full text-right text-xs">
            <thead class="bg-slate-50 text-slate-500 border-b">
              <tr>
                ...
                <th class="p-3">المسدد نقداً</th>
                <th class="p-3">المتبقي (الدين)</th>
                <th class="p-3">التاريخ</th>
                <th class="p-3 text-center">إجراء سداد</th>
              </tr>
            </thead>
      ...
      <section id="view-inventory" class="page-view hidden space-y-4">
        <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm">
          ...
          <div class="flex gap-2">
            ...
            <button onclick="openModal('modal-add-product')" class="bg-blue-600 text-white text-xs font-bold px-3 py-1.5 rounded-xl shadow">
              + إضافة صنف
            </button>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div class="bg-white p-3 rounded-2xl border shadow-sm">
            <span class="text-[11px] text-slate-400 font-bold">عدد الأصناف</span>
            <div class="text-xl font-black text-slate-800 mt-1">7 أصناف</div>
          </div>
          <div class="bg-white p-3 rounded-2xl border shadow-sm">
            <span class="text-[11px] text-slate-400 font-bold">قيمة المخزون بسعر الشراء</span>
            <div class="text-xl font-black text-blue-600 mt-1">146,800 ج.م</div>
          </div>
          <div class="bg-white p-3 rounded-2xl border shadow-sm">
            <span class="text-[11px] text-slate-400 font-bold">قيمة البيع المتوقعة</span>
            <div class="text-xl font-black text-emerald-600 mt-1">168,200 ج.م</div>
          </div>
          <div class="bg-white p-3 rounded-2xl border shadow-sm">
            <span class="text-[11px] text-slate-400 font-bold">النواقص (أقل من 5)</span>
            <div class="text-xl font-black text-rose-500 mt-1">1 صنف</div>
          </div>
        </div>

        <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-sm">
          <div class="p-3 border-b flex justify-between items-center">
            <input type="text" id="inventorySearch" placeholder="بحث في المخزن باسم الصنف..." class="border rounded-xl px-3 py-1.5 text-xs w-64" oninput="renderInventory()">
            <span class="text-xs text-slate-400">محدث لحظياً</span>
          </div>
          <table class="w-full text-right text-xs">
            <thead class="bg-slate-50 text-slate-500 border-b">
              <tr><th class="p-3">اسم الصنف</th><th class="p-3">القسم</th><th class="p-3">الرصيد</th><th class="p-3">سعر التكلفة</th><th class="p-3">سعر البيع</th><th class="p-3 text-center">إجراءات</th></tr>
            </thead>
      ...
      <!-- ================= 5. شاشة الهالك والمرتجعات (WASTE & RETURNS) ================= -->
      <section id="view-waste" class="page-view hidden space-y-4">
        <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm">
          <div>
            <h2 class="text-base font-black text-slate-800 flex items-center gap-2"><i data-lucide="alert-triangle" class="w-5 h-5 text-rose-500"></i> إدارة الهالك والمرتجعات (هالك عميل / هالك مورد)</h2>
            <p class="text-xs text-slate-400">تسجيل البضائع التالفة وفصل عيوب الاستخدام عن مرتجعات الموردين</p>
          </div>
          <button onclick="openModal('modal-add-waste')" class="bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs px-3.5 py-2 rounded-xl shadow">
            + تسجيل صنف هالك / مرتجع
          </button>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-4 gap-3">
          <div class="bg-white p-3 rounded-2xl border shadow-sm">
            <span class="text-[11px] text-slate-400 font-bold">هالك عميل (كسر / سوء استخدام)</span>
            <div class="text-lg font-black text-rose-600 mt-1" id="statWasteCust">1,450 ج.م</div>
          </div>
          <div class="bg-white p-3 rounded-2xl border shadow-sm">
            <span class="text-[11px] text-slate-400 font-bold">هالك مورد (عيوب صناعة RMA)</span>
            <div class="text-lg font-black text-amber-600 mt-1" id="statWasteSupp">3,200 ج.م</div>
          </div>
          <div class="bg-white p-3 rounded-2xl border shadow-sm">
            <span class="text-[11px] text-slate-400 font-bold">مرتجع مبيعات</span>
            <div class="text-lg font-black text-blue-600 mt-1" id="statRetSales">850 ج.م</div>
          </div>
          <div class="bg-white p-3 rounded-2xl border shadow-sm">
            <span class="text-[11px] text-slate-400 font-bold">مرتجع مشتريات</span>
            <div class="text-lg font-black text-emerald-600 mt-1" id="statRetPurch">0 ج.م</div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-sm">
          <table class="w-full text-right text-xs">
            <thead class="bg-slate-50 text-slate-500 border-b">
              <tr>
                <th class="p-3">نوع الهالك / المرتجع</th>
                <th class="p-3">الصنف / الجهاز</th>
                <th class="p-3">الكمية</th>
                <th class="p-3">القيمة المحسوبة</th>
      ...
      <!-- ================= 6. شاشة الهواتف والـ IMEI (DEVICES) ================= -->
      <section id="view-devices" class="page-view hidden space-y-4">
        <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm">
          <div>
            ...
            <p class="text-xs text-slate-400">تتبع السيريال نمبر، الأجهزة الجديدة والمستعملة، والضمان</p>
          </div>
          <button onclick="openModal('modal-add-device')" class="bg-blue-600 text-white font-bold text-xs px-3.5 py-2 rounded-xl shadow">
            + تسجيل جهاز جديد
          </button>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-sm">
          <table class="w-full text-right text-xs">
            <thead class="bg-slate-50 text-slate-500 border-b">
              <tr><th class="p-3">الموديل والجهاز</th><th class="p-3">الـ IMEI</th><th class="p-3">اللون والسعة</th><th class="p-3">الحالة</th><th class="p-3">سعر الشراء</th><th class="p-3">سعر البيع</th><th class="p-3">الضمان</th></tr>
            </thead>
      ...
      <!-- ================= 7. شاشة الإكسسوارات (ACCESSORIES) ================= -->
      <section id="view-accessories" class="page-view hidden space-y-4">
        <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm">
          <div>
            ...
            <p class="text-xs text-slate-400">شواحن، كابلات، سماعات، اسكرينات وجرابات</p>
          </div>
          <button onclick="openModal('modal-add-product')" class="bg-pink-600 text-white font-bold text-xs px-3.5 py-2 rounded-xl shadow">
            + إضافة إكسسوار
          </button>
        </div>
      ...
      <section id="view-repairs" class="page-view hidden space-y-4">
        <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm">
          <div>
            ...
            <p class="text-xs text-slate-400">كروت الاستلام، تشخيص الأعطال، وتسليم العملاء</p>
          </div>
      ...
      <section id="view-purchases" class="page-view hidden space-y-4">
        <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm">
          <div>
            ...
            <p class="text-xs text-slate-400">سجل توريد البضائع من الشركات والموزعين</p>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-sm">
          <table class="w-full text-right text-xs">
            <thead class="bg-slate-50 text-slate-500 border-b">
              <tr><th class="p-3">رقم الفاتورة</th><th class="p-3">المورد</th><th class="p-3">الأصناف</th><th class="p-3">الكمية</th><th class="p-3">الإجمالي</th><th class="p-3">طريقة الدفع</th><th class="p-3">التاريخ</th></tr>
            </thead>
      ...
      <section id="view-customers" class="page-view hidden space-y-4">
        <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm">
          <div>
            ...
            <p class="text-xs text-slate-400">البيانات، سجل المشتريات، والمديونيات الآجلة</p>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-sm">
          <table class="w-full text-right text-xs">
            <thead class="bg-slate-50 text-slate-500 border-b">
              <tr><th class="p-3">اسم العميل</th><th class="p-3">الهاتف</th><th class="p-3">العنوان</th><th class="p-3">إجمالي التعاملات</th><th class="p-3">الرصيد والآجل</th></tr>
            </thead>
      ...
      <section id="view-suppliers" class="page-view hidden space-y-4">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-sm">
          <table class="w-full text-right text-xs">
            <thead class="bg-slate-50 text-slate-500 border-b">
              <tr><th class="p-3">اسم الشركة / المورد</th><th class="p-3">الهاتف</th><th class="p-3">التصنيف</th><th class="p-3">إجمالي التوريدات</th><th class="p-3">المستحق للمورد</th></tr>
            </thead>
      ...
      <!-- ================= 12. شاشة الخزينة والمحافظ (TREASURY) ================= -->
      <section id="view-treasury" class="page-view hidden space-y-4">
        <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm">
          ...
          <button onclick="openModal('modal-wallet-transfer')" class="bg-emerald-600 text-white text-xs font-bold px-3 py-1.5 rounded-xl shadow">
            + تحويل / سحب كاش
          </button>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
          <div class="bg-white p-4 rounded-2xl border shadow-sm">
            <span class="text-xs text-slate-500 font-bold">خزينة المحل (كاش)</span>
            <div class="text-xl font-black text-slate-800 mt-1" id="cashBalText">1,240.00 ج.م</div>
          </div>
          <div class="bg-white p-4 rounded-2xl border shadow-sm">
            <span class="text-xs text-slate-500 font-bold">فودافون كاش</span>
            <div class="text-xl font-black text-blue-600 mt-1" id="vodafoneBalText">4,500.00 ج.م</div>
          </div>
          <div class="bg-white p-4 rounded-2xl border shadow-sm">
            <span class="text-xs text-slate-500 font-bold">أورنج / اتصالات / وي</span>
            <div class="text-xl font-black text-purple-600 mt-1">2,800.00 ج.م</div>
          </div>
          <div class="bg-white p-4 rounded-2xl border shadow-sm">
            <span class="text-xs text-slate-500 font-bold">انستاباي / بنك</span>
            <div class="text-xl font-black text-emerald-600 mt-1" id="bankBalText">8,300.00 ج.م</div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-sm p-4 space-y-3">
          ...
          <table class="w-full text-right text-xs">
            <thead class="bg-slate-50 text-slate-500 border-b">
              <tr><th class="p-2.5">النوع</th><th class="p-2.5">الحساب</th><th class="p-2.5">المبلغ</th><th class="p-2.5">العمولة (ربحك)</th><th class="p-2.5">الوقت</th></tr>
            </thead>
      ...
      <!-- ================= 13. شاشة التقارير والأرباح (REPORTS) ================= -->
      <section id="view-reports" class="page-view hidden space-y-4">
        <div class="flex items-center justify-between bg-white p-4 rounded-2xl border border-slate-200/80 shadow-sm">
          <div>
            <h2 class="text-base font-black text-slate-800 flex items-center gap-2"><i data-lucide="bar-chart-3" class="w-5 h-5 text-amber-500"></i> التقارير المالية وحساب الأرباح</h2>
      ...
      <section id="view-settings" class="page-view hidden space-y-4">
        <div class="bg-[#121c2e] text-slate-200 rounded-3xl border border-slate-800 shadow-2xl overflow-hidden flex flex-col md:flex-row min-h-[640px]">
          ...
          <div class="flex-1 p-5 md:p-6 overflow-y-auto custom-scroll space-y-4" id="settingsPanelsContainer">
            ...
            <div id="spane-printers" class="setting-pane hidden text-xs space-y-2">
              <h3 class="font-bold text-amber-400">إعدادات الطابعة الحرارية 80 مم</h3>
              <p class="text-slate-400">جاهزة للطباعة المباشرة التلقائية وقص الورق.</p>
            </div>

            <div id="spane-whatsapp" class="setting-pane hidden text-xs space-y-2">
              <h3 class="font-bold text-emerald-400">ربط واتساب المحل</h3>
              <p class="text-slate-400">الرقم المعتمد: 01070900711.</p>
            </div>
          </div>
        </div>
      </section>
  ...
  <!-- ================= MODAL: إتمام الدفع مع دعم الآجل والمحافظ ================= -->
  <div id="modal-checkout-pos" class="fixed inset-0 bg-black/75 backdrop-blur-sm z-55 hidden flex items-center justify-center p-3">
    <div class="bg-white rounded-3xl w-full max-w-md shadow-2xl p-5 space-y-3.5 text-xs">
      ...
      <!-- حقول الدفع الآجل (تظهر فقط عند اختيار آجل) -->
      <div id="creditFields" class="hidden space-y-2 p-3 bg-amber-50/70 border border-amber-200 rounded-2xl">
        <div class="grid grid-cols-2 gap-2">
          <div>
            <label class="text-[10px] text-slate-500 font-bold block mb-1">المبلغ المدفوع الآن:</label>
          ...
          <div>
            <label class="text-[10px] text-slate-500 font-bold block mb-1">المتبقي دين على العميل:</label>
        ...
        <div>
          <label class="text-[10px] text-slate-500 font-bold block mb-1">اسم العميل صاحب الدين (إجباري للآجل):</label>
          <input type="text" id="creditCustName" placeholder="اكتب اسم العميل الثلاثي" class="w-full border rounded-xl p-2 font-bold">
        </div>
        <div>
          <label class="text-[10px] text-slate-500 font-bold block mb-1">رقم هاتف العميل:</label>
          <input type="text" id="creditCustPhone" placeholder="010XXXXXXXX" class="w-full border rounded-xl p-2 font-mono">
        </div>
      </div>
  ...
  <div id="modal-pay-debt" class="fixed inset-0 bg-black/75 backdrop-blur-sm z-55 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl w-full max-w-sm p-5 shadow-2xl space-y-3 text-xs">
      <div class="flex justify-between items-center border-b pb-2">
        <h3 class="font-black text-sm text-slate-800">تحصيل وسداد دفعة من الدين</h3>
      ...
      <div>
        <span class="text-slate-400 block">اسم العميل:</span>
        <span class="font-black text-sm text-slate-800" id="payDebtCustName"></span>
      </div>
      <div>
        <span class="text-slate-400 block">إجمالي الدين المتبقي:</span>
        <span class="font-black text-base text-rose-600" id="payDebtRemainVal"></span>
      </div>
      <div>
        <label class="text-slate-500 font-bold block mb-1">المبلغ المحصل الآن:</label>
        <input type="number" id="payDebtInput" class="w-full border rounded-xl p-2 font-black text-emerald-600 text-base">
      </div>
      <button onclick="confirmSettleDebt()" class="w-full bg-emerald-600 text-white font-black py-2.5 rounded-xl shadow">
        تسجيل السداد وإيداع المبلغ بالخزينة
      </button>
    </div>
  </div>

  <!-- ================= MODAL: تسجيل صنف هالك أو مرتجع ================= -->
  <div id="modal-add-waste" class="fixed inset-0 bg-black/75 backdrop-blur-sm z-55 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl w-full max-w-md p-5 shadow-2xl space-y-3 text-xs">
      ...
      <div>
        <label class="text-slate-500 font-bold block mb-1">نوع العملية والتصنيف:</label>
        <select id="wasteTypeSelect" class="w-full border rounded-xl p-2 font-bold">
          ...
          <option value="مرتجع مبيعات (من عميل)">مرتجع مبيعات (استرجاع سليم من عميل)</option>
          <option value="مرتجع مشتريات (لمورد)">مرتجع مشتريات (إرجاع للشركة)</option>
        </select>
      </div>
      <div>
        <label class="text-slate-500 font-bold block mb-1">اسم الصنف أو الجهاز:</label>
        <input type="text" id="wasteItemName" placeholder="مثال: شاشة سامسونج A12 أصلية أو هاتف..." class="w-full border rounded-xl p-2 font-bold">
      </div>
      <div class="grid grid-cols-2 gap-2">
        <div>
          <label class="text-slate-500 font-bold block mb-1">الكمية التالفة:</label>
          <input type="number" id="wasteQty" value="1" min="1" class="w-full border rounded-xl p-2 font-bold">
        </div>
        <div>
          <label class="text-slate-500 font-bold block mb-1">قيمة التكلفة الإجمالية:</label>
          <input type="number" id="wasteCost" value="650" class="w-full border rounded-xl p-2 font-bold text-rose-600">
        </div>
      </div>
      <div>
        <label class="text-slate-500 font-bold block mb-1">السبب وتفاصيل التلف:</label>
        <input type="text" id="wasteReason" placeholder="مثال: كسر فلاتة داخلية أثناء الفحص" class="w-full border rounded-xl p-2">
      </div>
      <button onclick="saveWasteRecord()" class="w-full bg-rose-600 hover:bg-rose-700 text-white font-black py-2.5 rounded-xl shadow mt-2">
        حفظ في سجل الهالك والمرتجع
      </button>
    </div>
  </div>
    ...
    <div class="bg-[#121c2e] text-slate-100 border border-slate-800 rounded-3xl w-full max-w-sm p-5 shadow-2xl space-y-3.5 text-xs">
      <div class="flex justify-between items-center border-b border-slate-800 pb-2">
        <h3 class="font-black text-sm text-blue-400 flex items-center gap-1.5">
          <i data-lucide="key-round" class="w-4 h-4"></i> تعديل باسوورد المستخدم
        </h3>
      ...
      <div>
        ...
        <input type="text" id="editUserName" class="w-full bg-[#16233b] border border-slate-700 rounded-xl p-2 text-slate-100 font-bold">
      </div>
      <div>
        <label class="text-slate-400 block mb-1">اسم المستخدم (Login):</label>
        <input type="text" id="editUserLogin" class="w-full bg-[#16233b] border border-slate-700 rounded-xl p-2 text-slate-100 font-mono font-bold">
      </div>
      <div>
        <label class="text-amber-400 block mb-1 font-bold">كلمة المرور الجديدة:</label>
        <input type="text" id="editUserPass" placeholder="اكتب الباسوورد الجديد..." class="w-full bg-[#16233b] border border-amber-500/50 rounded-xl p-2 text-amber-300 font-mono font-bold">
      </div>
      <button onclick="saveUserPasswordChange()" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-2.5 rounded-xl shadow-lg mt-2">
        حفظ وتحديث الباسوورد
      </button>
    </div>
  </div>
  ...
  <!-- ================= JAVASCRIPT ENGINE ================= -->
  <script>
    // قاعدة البيانات الشاملة لجميع الأقسام
    const initialDatabase = {
      ...
      wasteRecords: [
        ...
        { type: 'هالك عميل (كسر / سوء استخدام)', item: 'اسكرينة 11D سيراميك', qty: 2, cost: 50, reason: 'كسر أثناء التركيب بسبب ضغط خاطئ', date: '2026-09-05' }
      ],
      purchases: [
    ...
    let App = JSON.parse(localStorage.getItem('EL_RESALA_ENTERPRISE_V4_STORE')) || initialDatabase;

    function saveApp() {
      localStorage.setItem('EL_RESALA_ENTERPRISE_V4_STORE', JSON.stringify(App));
    }
      ...
      if (ctx1) {
        ...
        salesChartInstance = new Chart(ctx1, {
          ...
          data: {
            ...
            datasets: [
              { label: 'إجمالي المبيعات (ج.م)', data: [8400, 11200, 9500, 14200, 12800, 18500, 12450], borderColor: '#2563eb', fill: true, backgroundColor: 'rgba(37, 99, 235, 0.08)', tension: 0.4, borderWidth: 3 },
              { label: 'صافي الأرباح (ج.م)', data: [2100, 2800, 2400, 3600, 3100, 4800, 3200], borderColor: '#10b981', fill: true, backgroundColor: 'rgba(16, 185, 129, 0.05)', tension: 0.4, borderWidth: 2.5, borderDash: [4, 4] }
            ]
          },
    ...
    // ================= 1. محرك المبيعات (POS) وتخصيص الأسعار =================
    function renderPos() {
    ...
    // تعديل السعر المخصص مباشرة من السلة
    function updateCartItemPrice(index, newPrice) {
      const val = parseFloat(newPrice) || 0;
      App.cart[index].price = val;
      saveApp();
      renderCart(false);
    }
    ...
    function renderCart(renderItems = true) {
          ...
          box.innerHTML += `
            <div class="p-2.5 rounded-xl bg-slate-50 border space-y-1.5 text-xs">
              ...
              <div class="flex items-center justify-between gap-2 pt-1 border-t border-slate-200/60">
                ...
                <div class="flex items-center gap-1">
                  <span class="text-slate-400">سعر البيع (مخصص):</span>
      ...
      let sum = 0;
      App.cart.forEach(i => sum += (i.price * i.qty));
    ...
    function confirmFinalCheckout() {
      ...
      if (mode === 'CREDIT') {
        ...
        if (!cust) return alert('يرجى كتابة اسم العميل لتسجيل الفاتورة الآجلة في حسابه!');
        ...
        alert(`✅ تم إتمام الفاتورة الآجلة بنجاح!\nالمسدد كاش: ${paid} ج.م\nالمتبقي دين على العميل (${cust}): ${remain} ج.م\nتمت إضافتها لصفحة الديون والآجل.`);
      } else {
        alert(`✅ تم تسجيل الفاتورة بنجاح عبر طريقة الدفع: [${mode}] وإيداع المبلغ.`);
      }
    ...
    // ================= 2. محرك الديون والآجل (DEBTS ENGINE) =================
    function renderDebts() {
        ...
        tbody.innerHTML += `
          <tr class="hover:bg-slate-50">
            ...
            <td class="p-3 text-center">
              <button onclick="openPayDebtModal(${idx})" class="bg-blue-50 hover:bg-blue-100 text-blue-600 border border-blue-200 px-3 py-1 rounded-xl font-black text-xs transition">
                سداد دفعة
              </button>
            </td>
          </tr>
      ...
      document.getElementById('totalDebtsAmount').innerText = `${totalRemain.toLocaleString()} ج.م`;
      document.getElementById('totalDebtsCount').innerText = `${App.debts.length} فاتورة`;
    }
    ...
    function confirmSettleDebt() {
      ...
      if (d.remaining <= 0) {
        alert(`🎉 تم سداد كامل الدين للفاتورة (${d.id}) الخاصة بالعميل (${d.customer}) بنجاح!`);
        App.debts.splice(idx, 1);
      } else {
        alert(`✅ تم سداد مبلغ (${val} ج.م) بنجاح. المتبقي في حساب العميل: ${d.remaining} ج.م.`);
      }
    ...
    // ================= 3. محرك الهالك والمرتجعات (WASTE & RETURNS) =================
    function renderWaste() {
      const tbody = document.getElementById('wasteTableBody');
      if (!tbody) return;
      tbody.innerHTML = '';

      let cW = 0, sW = 0, sR = 0, pR = 0;
      App.wasteRecords.forEach(w => {
        if (w.type.includes('هالك عميل')) cW += w.cost;
        if (w.type.includes('هالك مورد')) sW += w.cost;
        if (w.type.includes('مرتجع مبيعات')) sR += w.cost;
        if (w.type.includes('مرتجع مشتريات')) pR += w.cost;
      ...
      document.getElementById('statRetSales').innerText = `${sR.toLocaleString()} ج.م`;
      document.getElementById('statRetPurch').innerText = `${pR.toLocaleString()} ج.م`;
    }
    ...
    function saveWasteRecord() {
      ...
      alert('✅ تم حفظ قيد الهالك / المرتجع بنجاح.');
    }

    // ================= 4. باقي دوال الأقسام والمخزون =================
    function renderInventory() {
      const tbody = document.getElementById('inventoryTableRows');
      if (!tbody) return;
      tbody.innerHTML = '';
      const q = (document.getElementById('inventorySearch')?.value || '').toLowerCase();
      App.products.filter(p => p.name.toLowerCase().includes(q)).forEach(p => {
        tbody.innerHTML += `
          <tr class="hover:bg-slate-50">
            ...
            <td class="p-3 font-black ${p.stock <= 4 ? 'text-rose-500' : 'text-emerald-600'}">${p.stock}</td>
            <td class="p-3 text-slate-600">${p.cost} ج.م</td>
            <td class="p-3 font-black text-slate-900">${p.price} ج.م</td>
            <td class="p-3 text-center">
              <button onclick="p.stock = parseInt(prompt('تعديل الرصيد:', p.stock)); saveApp(); renderInventory();" class="text-blue-600 font-bold hover:underline">تعديل</button>
            </td>
          </tr>
        `;
      });
    ...
    // ================= 5. تعديل باسوورد المدير والمستخدمين =================
...
git commit -m "feat: complete professional enterprise POS with custom price override, multi-wallets, dedicated debts tracking, customer/supplier waste RMA, and editable admin credentials"
git push origin main

echo "=========================================================="
echo "✨ تم رفع وتحديث النظام بالكامل بنجاح!"
echo "كل الشاشات تعمل الآن بالبيانات الكاملة وبدون أي شاشة بيضاء أو فارغة."
echo "=========================================================="
